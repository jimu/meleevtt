...
export class EntitySheetHelper {
      ...
      if ( el.classList.contains("attribute-key") ) {
        ...
        for ( let i = 0; i < groups.length; i++ ) {
          if (groups[i].value === val) {
            ui.notifications.error(game.i18n.localize("MELEE.NotifyAttrDuplicate") + ` (${val})`);
            el.value = oldVal;
            attrError = true;
            break;
          }
  ...
  static onAttributeRoll(event) {
    ...
    const shorthand = game.settings.get("melee", "macroShorthand");
  ...
  static validateGroup(groupName, document) {
    ...
    if ( groups.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("MELEE.NotifyGroupDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for group keys that match attribute keys.
    if ( attributes.includes(groupName) ) {
      ui.notifications.error(game.i18n.localize("MELEE.NotifyGroupAttrDuplicate") + ` (${groupName})`);
      return false;
    }

    // Check for whitespace or periods.
    if ( groupName.match(/[\s|\.]/i) ) {
      ui.notifications.error(game.i18n.localize("MELEE.NotifyGroupAlphanumeric"));
      return false;
    }
    return true;
  }
  ...
  static async deleteAttributeGroup(event, app) {
    ...
    new Dialog({
      title: game.i18n.localize("MELEE.DeleteGroup"),
      content: `${game.i18n.localize("MELEE.DeleteGroupContent")} <strong>${group.val()}</strong>`,
      buttons: {
        confirm: {
  ...
  static async createDialog(data={}, options={}) {
    ...
    const templates = collection.filter(a => a.getFlag("melee", "isTemplate"));
    const defaultType = this.metadata.types[0];
    const types = {
      [defaultType]: game.i18n.localize("MELEE.NoTemplate")
    }
    ...
    return Dialog.prompt({
      ...
      callback: html => {
        ...
        if ( template ) {
          ...
          delete createData.flags.melee.isTemplate;
        }
